...
print(
    4 ** 3)  # Возведение в степень Кого возводим в какую степень  ВОЗВЕДЕНИЕ В СТЕПЕНЬ Многократное увеличение числа на себя записывается **
...
h = 2 << 2;
print(
    h)  # 2 << 2 даст 8 . В двоичном виде 2 представляет собой 10 .Сдвиг влево на 2 бита даёт1000 , что в десятичном виде означает 8 .
i = 11 >> 1;
print(
    i)  # 11 >> 1 даст 5 . В двоичномвиде 11 представляется как1011 , что будучи смещённымна 1 бит вправо, даёт 101 , а это,в свою очередь, ни что иноекак десятичное 5

j = 5 & 3;
print(j)  # Побитовая операция И над числами            1
k = 5 | 3;
print(k)  # Побитовая операция ИЛИ над числами         7
l = 5 ^ 3;
print(l)  # Побитовая операцияИСКЛЮЧИТЕЛЬНО ИЛИ         6
m = ~5;
print(m)  # Побитовая операция НЕ для числа x соответствует -(x+1) ~5 даёт -6
n = 3 < 5;
print(n)  # True
...
print((9 > 7) and (
            2 < 4))  # Оба выражения истинны (True)           Логи-ческое И    Оператор and оценивает выражение как истинное только тогда, когда оба компонента истинны.
